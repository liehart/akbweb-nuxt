      ...
      <div class="flex justify-between">
        <div class="flex-col my-auto">
          <h1 class="text-4xl font-bold">
            Tambah Jabatan Pegawai Baru
          </h1>
          <p>Menu untuk menambah jabatan pegawai baru</p>
        </div>
...
export default {
  ...
  methods: {
    ...
    createRoleAPI () {
      this.$axios.post('role', this.form).then(() => {
        this.back()
        this.$toast.show({
          type: 'success',
          title: 'Berhasil',
          message: 'Data jabatan ' + this.form.name + ' berhasil dibuat.',
          timeout: 3
        })
      }).catch((err) => {
        if (err.response) {
