    ...
    <div class="mb-5 antialiased">
      <div class="flex justify-between">
        <div class="flex-col my-auto">
          ...
          <p>Menu untuk mengedit pelanggan</p>
        </div>
...
export default {
  ...
  async fetch () {
    await this.$axios.get('customer/' + this.$route.params.id).then((res) => {
  ...
  methods: {
    back () {
      this.$router.push('/dashboard/pelanggan/' + this.$route.params.id)
    },
    createCustomerAPI () {
      this.$axios.put('customer/' + this.form.id, this.form).then((res) => {
        this.back()
        this.$toast.show({
          type: 'success',
          title: 'Berhasil',
          message: 'Data pelanggan ' + this.form.name + ' berhasil diedit.',
          timeout: 3
        })
      }).catch((err) => {
        if (err.response) {
