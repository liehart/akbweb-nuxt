        ...
        <div class="bg-white rounded-md shadow w-full h-full">
          <div class="bg-gray-50 px-5 py-3 rounded-t-md border-b font-bold text-gray-700">
            ...
            <p class="text-xs font-light text-gray-500">
              Bagian ini berisi informasi dasar tentang menu yang ingin diubahkan.
            </p>
          </div>
...
export default {
  ...
  async fetch () {
    await this.$axios.get('menu/' + this.$route.params.id).then((res) => {
  ...
  head () {
    return {
      title: 'Menu ' + this.title + ' | Atma Korean BBQ'
    }
  },
  methods: {
    ...
    createMenuAPI () {
      this.$axios.put('menu/' + this.form.id, this.form).then(() => {
        ...
        this.$toast.show({
          type: 'success',
          title: 'Berhasil',
          message: 'Menu ' + this.form.name + ' berhasil diubah.',
          timeout: 3
        })
      }).catch((err) => {
        if (err.response) {
